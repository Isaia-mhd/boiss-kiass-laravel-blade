...
@section('adminContent')
    <div class="w-full ">
        <div class="w-full">
            <h1 class="text-start text-white text-2xl font-semibold mb-6">All article page</h1>
        </div>
        <div class="mr-6">
            <p>
                @include('notify.success')
            </p>
            <table class="w-full   mx-auto rounded-tr-xl rounded-tl-xl table-auto border-collapse">
                <thead>
                ...
                <tbody>
                    @foreach($articles as $article)
                        <tr class="text-center text-sm border shadow-lg">
                            <td class="text-white border py-2 border-y-8 border-x-0  border-y-white">{{ $article->id }}</td>
                            ...
                            <td class="text-white border py-2 border-y-8 border-x-0  border-y-white">{{ $article->name }}</td>
                            <td class="text-white border py-2 border-y-8 border-x-0  border-y-white">${{ $article->price }}</td>
                            <td class="text-white border py-2 border-y-8 border-x-0  border-y-white">{{ $article->short_description }}</td>
